...
protocol saveRecipeToPersDelegate{
    func savePers(recipe: PersonalRecipe)
}
protocol PersonalRecipeSaver {
...
class CreateRecipeViewController: UIViewController, UITextFieldDelegate,UITextViewDelegate {
    ...
    var delegate: saveRecipeDelegate?
    var delegatePers: saveRecipeToPersDelegate?
    var saveRecipeClosure: ((PersonalRecipe) -> Void)?
    var personalRecipeSaverDelegate: PersonalRecipeSaver?
    ...
    override func viewDidLoad() {
        ...
        if let image = sharedImage {
            // Update your image view or other UI components with the shared image
    ...
    func setUIColor() {
        ...
        saveButton.setTitleColor(isDarkMode ? textColorDark : textColorDark, for: .normal)
    ...
    @objc private func saveRecipe() {
        ...
        recipe.title = title
        recipe.descrip = description
        recipe.timeSpent = timeSpent
        recipe.image = image?.resizeImageWith(maxSize: 1280).jpegData(compressionQuality: 0.7) // Save image as Data
        
        // Create Ingredients objects and set relationship
        let ingredientsSet = NSSet(array: ingredientsList.map { ingredientString in
            let ingredient = Ingredients(context: self.context)
        ...
        ShowPersonalRecipeView.shared.saveRecipeWithImageToFirebase(recipe: personalRecipe)
        personalRecipeSaverDelegate?.saveRecipeWithImageToFirebase(recipe: personalRecipe)
        saveRecipeClosure?(personalRecipe)
        // Save to Core Data
        do {
            try context.save()
            delegate?.saveRecipe(recipe: recipe)
            delegatePers?.savePers(recipe: personalRecipe)
        } catch {
...
@available(iOS 13.0, *)
extension CreateRecipeViewController: FormSheetDelegate {
...
@available(iOS 13.0, *)
extension CreateRecipeViewController: AddIngredientDelegate{
